...
export class CharacterFactory {
  public readonly characterCache: { [key: string]: IPrintableFlyweight } = <
    any
    >{};
  ...
  getCharacter(
    ...
    if (this.characterCache[characterKey]) {
      if (enableDebug) {
        terminal.print(
          `Using existing flyweight for ${characterKey}`, chalk.blue
        );
      }
      ...
      if (enableDebug) {
        terminal.print(
          `Existing flyweight for ${characterKey} not found, creating`, chalk.red
        );
      }
      const newChar: IPrintableFlyweight = new TextFlyweight(
        name,
        fontFamily,
        fontColor
      );
  ...
  printCharacterCache(includeCount: boolean = true, fontColor?: FontColor): void {
    ...
    for (const key in this.characterCache) {
      terminal.print(` ${key}`, fontColor);
      flyweightCount++
    }

    if (includeCount) { terminal.print(`${flyweightCount} total flyweights`, fontColor) }
  }
}
